...
import { getLatestBlogs, getTimeAgo, PostRecord } from "@/lib/content";
...
function BlogCard({ post, locale, badgeLabel, byText, readingTimeText, isPriority = false }: BlogCardProps) {
  return (
    <Link href={post.url} className="block">
      <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer rounded-none pb-0 h-auto gap-2">
...
export async function LatestEdition({ locale, blogCount = 2 }: LatestEditionProps) {
  ...
  const blogPosts = await getLatestBlogs(locale, blogCount);
