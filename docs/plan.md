...
2. **Contentlayer Configuration**
   ...
   - Create `contentlayer.config.ts` that:
     ...
     - Defines Post document with fields: title, slug, summary, date, updated (optional), locale (en, es, fr, it, pt), tags (string list), coverImage (optional), heroVideo (optional), author, translation_key, type ('news' | 'blog'), featured (boolean), published (boolean)
...
4. **i18n Helper**
   ...
   - Create `lib/i18n.ts` that exports: locales array [en, es, fr, it, pt], Locale type, and defaultLocale as 'en'
